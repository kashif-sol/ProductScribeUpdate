			...
			<div class="card-body py-4">
				<div class="row align-items-center">
					<div class="col-6 col-md-4">
						<div class="row" style="align-items: center;">
							...
							<div class="col-sm-6">
								...
								<strong class="text-6 text-color-dark" style="float: left;">{{$remaining_credits}}</strong>
							</div>
						</div>
					</div>
					...
					<div class="col-6 col-md-4 border border-top-0 border-end-0 border-bottom-0 border-color-light-grey py-3">
						...
						<a href="{{route('plans')}}" class="btn btn-default" style="background-color: #191C21; border: none; color: white; margin-top: 10px;">Click here <i class="fa-solid fa-arrow-right"></i></a>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="row">
		<div class="col">
			...
			<div class="card card-modern card-modern-table-over-header shadow p-3 mb-5 bg-white rounded">
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							...
							<div class="card-body"  style="padding: 0px;">
								<div id="loadingCard" class="card-body" data-loading-overlay data-loading-overlay-options='{ "startShowing": false, "css": { "backgroundColor": "transparent" } }' style="min-height: 150px;">
				...
				<tbody>
					@foreach($products as $key=>$product)
						<tr>
                            ...
                            <td width="60px"><img src="{{$product['images'][0]['src']}}" height="60px" width="60px" alt="description of myimage" style="border-radius: 4px;"></td>
                            @else
                            <td><img src="{{asset('Images/NoImage.png')}}" height="60px" width="60px" alt="description of myimage"></td>
                            @endif
							<td>
								<div class="row">
								...
								<div class="row">
									<label style="color: #6F6F91;">Price: {{$product['variants']['0']['price']}}</label> 
								</div>
								
							</td>
							<td class="editButton" style="vertical-align: middle;">
								<button data-id="{{$product['id']}}" data-title="{{$product['title']}}" class="mb-1 mt-1 me-1 btn btn-default" data-bs-toggle="modal" data-bs-target="#modalBootstrap" style="border-radius: 5%; background-color: #191C21; color: white; border-radius: 3px;">Title</button>
							</td>
							<td class="descriptionButton" style="vertical-align: middle;">
								<button data-id="{{$product['id']}}" data-title="{{$product['title']}}" class="mb-1 mt-1 me-1 btn btn-default" data-bs-toggle="modal" data-bs-target="#modalBootstrap" style="border-radius: 5%; background-color: #191C21; color: white; border-radius: 3px;">Description</button>
							</td>
						</tr>
					@endforeach
				</tbody>
				...
				<div class="row">
					@if(isset($link["previous"]) && isset($link["next"]))
						<div class="col-sm-2">
							<a href="{{url('/view-products?pageInfo='.$link["previous"])}}" class="btn btn-primary"><i class="fa-solid fa-arrow-left"></i> Previous</a>
						</div>
						<div class="col-sm-2">
							<a href="{{url('/view-products?pageInfo='.$link["next"])}}" class="btn btn-primary">Next  <i class="fa-solid fa-arrow-right"></i></a>
						</div>
					@else
						@if(isset($link["next"]))
							<div class="col-sm-2">
...
<script>
	...
	$(document).on('click', '#generateKeyword', function () {
		var $el = $('#loadingCard');
		var productTitle = $('#model_product_title').html();
		var user_id ='<?php echo $user_id; ?>';
        $el.trigger('loading-overlay:show');
		$.ajax({
            url: '/generate-keyword',
            type: 'POST',
			data: {productTitle : productTitle, user_id : user_id},
            success: function (response) {
				$el.trigger('loading-overlay:hide');
				if(response[1] == 200){
					$('#keywords').val(response[0]);
				}else{
					showMonoErrorAlert($('#keywords').val(response[0]));
				}
            },
	...
	function formFieldTemplete(form, removeButton){
		...
		if(form == 'title'){
			...
			if(removeButton == 0){
				...
				html += '<input id="keywords" type="text" placeholder="Type keywords here" class="form-control" name="keywords[]">';
				html += '</div>';
				html += '<div class="col-sm-3" style="padding-left:0px;">';
				html += '<button id="generateKeyword" type="button" class="btn btn-default" style="background-color: #191C21; border: none; color: white;">Generate</button>';
				html += '</div>';
				html += '</div>';
			}
			html += '</div>';
			...
			html += '<div class="col-lg-8" style="padding-bottom:4px;">';
		...
		if(form == 'description'){
			...
			html += '<label class="col-lg-3 control-label text-lg-end pt-1" for="textareaDefault">Example Title</label>';
			html += '<div class="col-lg-8">';
			html += '<input type="text" placeholder="Type example title here" class="form-control pt-1" name="exampleTitles[]">';
			if(removeButton == 1){
				html += '<div class="input-group-append">';
				html += '<button id="removeRow" type="button" class="btn btn-danger" style="float:right; margin-top:5px">Delete <i class="fa-solid fa-arrow-up"></i></button>';
				html += '</div>';
